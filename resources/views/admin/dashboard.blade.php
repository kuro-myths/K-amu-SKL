...
<section class="py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            ...
            <p class="text-gray-500 text-sm">Ikhtisar platform K-amu SKL</p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
                ...
                <p class="text-sm text-gray-500">Total Tautan</p>
            </div>
            <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 card-hover">
                ...
                <p class="text-sm text-gray-500">Tautan Unggulan</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <a href="{{ route('admin.educations.pending') }}" class="bg-white rounded-2xl p-5 border border-gray-100 card-hover flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-900">Tinjau Tertunda</h3>
                    <p class="text-sm text-gray-500">{{ $pendingEducations }} tautan menunggu</p>
                </div>
            </a>
            ...
            <a href="{{ route('admin.users.index') }}" class="bg-white rounded-2xl p-5 border border-gray-100 card-hover flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-900">Kelola Pengguna</h3>
                    <p class="text-sm text-gray-500">{{ $totalUsers }} pengguna terdaftar</p>
                </div>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100">
            <h2 class="text-lg font-bold text-gray-900 mb-4">Tautan Menunggu Terkini</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                    ...
                    <div class="flex gap-2">
                        <form action="{{ route('admin.educations.approve', $edu) }}" method="POST">
                            @csrf @method('PATCH')
                            <button class="px-3 py-1 bg-green-50 text-green-600 rounded-lg text-sm font-medium hover:bg-green-100 transition">Setujui</button>
                        </form>
                        <form action="{{ route('admin.educations.reject', $edu) }}" method="POST">
                            @csrf @method('PATCH')
                            <button class="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-sm font-medium hover:bg-red-100 transition">Tolak</button>
                        </form>
                    </div>
                </div>
            @endforeach
