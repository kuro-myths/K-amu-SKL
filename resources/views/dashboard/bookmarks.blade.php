@extends('layouts.app')
@section('title', 'Tautan Tersimpan')
...
<section class="py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tautan Tersimpan</h1>
            ...
            <a href="{{ route('dashboard') }}" class="text-gray-500 hover:text-purple-600 text-sm flex items-center">
                <i data-feather="arrow-left" class="w-4 h-4 mr-1"></i> Dasbor
            </a>
        </div>
        ...
        @if($bookmarks->isEmpty())
            <div class="text-center py-20">
                ...
                <p class="text-gray-500 text-lg">Belum ada tautan tersimpan.</p>
                <a href="{{ route('explore') }}" class="text-purple-600 font-semibold mt-2 inline-block">Jelajahi sumber belajar</a>
            </div>
        @else
                ...
                @foreach($bookmarks as $bm)
                    <div class="bg-white border border-gray-100 rounded-2xl p-6 card-hover transition-all duration-300">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <span class="px-2 py-1 rounded-lg bg-teal-50 text-teal-600 text-xs font-medium">{{ $bm->education->level_label }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('education.show', $bm->education) }}" class="text-purple-600 text-sm font-semibold hover:text-purple-700 transition flex items-center">
                                Rincian <i data-feather="arrow-right" class="w-4 h-4 ml-1"></i>
                            </a>
                        </div>
                    </div>
                @endforeach
